...
func (s *URLStore) Load(ctx context.Context) error {
	...
	select {
	case <-ctx.Done():
		return model.NewStoreError("LoadCanceled", "", model.ErrCanceled)
	default:
	}
...
func (s *URLStore) Flush() error {
	...
	if !s.ready.Load() {
		return model.NewStoreError("FlushNotReady", "", model.ErrStoreNotReady)
	}
	return s.sync()
}
...
func (s *URLStore) Get(code string) (*model.ShortURL, error) {
	if !s.ready.Load() {
		return nil, model.NewStoreError("FetchURLNotReady", code, model.ErrStoreNotReady)
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
	u, ok := s.urls[code]
	if !ok {
		return nil, model.NewStoreError("FetchURL", code, model.ErrCodeNotFound)
	}
	return u, nil
}
...
func (s *URLStore) Exists(code string) (bool, error) {
	if !s.ready.Load() {
		return false, model.NewStoreError("CheckExistsNotReady", code, model.ErrStoreNotReady)
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
	_, ok := s.urls[code]
...
func (s *URLStore) Save(u *model.ShortURL, overwrite bool) error {
	...
	if !s.ready.Load() {
		return model.NewStoreError("SaveURLNotReady", u.Code, model.ErrStoreNotReady)
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	if _, ok := s.urls[u.Code]; ok && !overwrite {
		return model.NewStoreError("SaveURL", u.Code, model.ErrCodeConflict)
	}
	clone := *u
	s.urls[u.Code] = &clone
	s.dirty.Store(true)
...
func (s *URLStore) Delete(code string) error {
	if !s.ready.Load() {
		return model.NewStoreError("DeleteURLNotReady", code, model.ErrStoreNotReady)
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	if _, ok := s.urls[code]; !ok {
		return model.NewStoreError("DeleteURL", code, model.ErrCodeNotFound)
	}
	delete(s.urls, code)
	s.dirty.Store(true)
	if s.flushOn {
...
func (s *URLStore) IncrementVisits(code string) (*model.ShortURL, error) {
	if !s.ready.Load() {
		return nil, model.NewStoreError("BumpVisitsNotReady", code, model.ErrStoreNotReady)
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	u, ok := s.urls[code]
	if !ok {
		return nil, model.NewStoreError("BumpVisits", code, model.ErrCodeNotFound)
	}
	u.Visits++
...
func (s *URLStore) ForEach(fn func(u *model.ShortURL) bool) error {
	if !s.ready.Load() {
		return model.NewStoreError("ForEachNotReady", "", model.ErrStoreNotReady)
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *URLStore) ListCodes(limit int) ([]*model.ShortURL, error) {
	if !s.ready.Load() {
		return nil, model.NewStoreError("ListCodesNotReady", "", model.ErrStoreNotReady)
	}
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *URLStore) Stats() (total, active, disabled, expired int, err error) {
	if !s.ready.Load() {
		return 0, 0, 0, 0, model.NewStoreError("StatsNotReady", "", model.ErrStoreNotReady)
	}
	now := time.Now()
	s.mu.RLock()
	defer s.mu.RUnlock()
